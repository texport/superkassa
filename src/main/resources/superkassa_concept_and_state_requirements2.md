...
### 9.1 ZXReport: счётчики, агрегаты и контрольные суммы

- **Источник данных ZXReport**: агрегаты строятся в `superkassa-core` по счётчикам:
  - операции: `operation.OPERATION_*.count/sum`;
  - секции: `section.%s.operation.OPERATION_*.count/sum`;
  - билеты/платежи: `ticket.OPERATION_*.total_count/count/sum`, `ticket.OPERATION_*.payment.PAYMENT_*.count/sum`;
  - внесения/изъятия: `money_placement.MONEY_PLACEMENT_*.total_count/count/sum/offline_count`;
  - налоги: `tax.VAT_*.OPERATION_*.turnover/turnover_without_tax/sum`;
  - необнуляемые суммы: `start_shift_non_nullable.OPERATION_*.sum` и `non_nullable.OPERATION_*.sum`.
- **Необнуляемые суммы по смене**:
  - при **открытии смены** в счётчики `start_shift_non_nullable.OPERATION_*.sum` (scope=SHIFT) копируются глобальные необнуляемые суммы `non_nullable.OPERATION_*.sum` (scope=GLOBAL);
  - в ZXReport поле `startShiftNonNullableSums` всегда содержит полный набор операций (`SELL/SELL_RETURN/BUY/BUY_RETURN`), отсутствующие значения заполняются нулями;
  - поле `nonNullableSums` отражает конечное состояние: если есть явный `non_nullable.OPERATION_*.sum` в счётчиках смены — используется он, иначе считается как `start_shift_non_nullable + operation.OPERATION_*.sum`.
- **Полнота массивов ZXReport**:
  - `operations`, `discounts`, `markups`, `totalResult`, `ticketOperations`, `moneyPlacements`, `nonNullableSums`, `startShiftNonNullableSums`, `taxes` всегда сериализуются с фиксированным количеством элементов:
    - по одной записи на каждую операцию (`SELL/SELL_RETURN/BUY/BUY_RETURN`);
    - по одной записи на каждую группу НДС (`VatGroup`) в блоке `taxes`;
    - по одной записи на каждую комбинацию операция × тип оплаты в `ticketOperations[*].payments`;
    - секции (`sections`) содержат динамический набор кодов отделов, но **внутри каждой секции** всегда есть полный набор операций;
  - при отсутствии движения по операции/налогу/типу оплаты все счётчики в элементе заполняются нулями, элемент **не удаляется**.
- **Расчёт итогов по операциям**:
  - массив `totalResult` строится из `operation.OPERATION_*.sum` с корректировкой на скидки/наценки:
    - `total = OPERATION_SUM - DISCOUNT_SUM + MARKUP_SUM`;
  - для каждой операции в `operations`, `discounts`, `markups`, `totalResult` используется один и тот же набор кодов операций.
- **Checksum Z‑отчёта**:
  - при построении Z‑отчёта JSON-блок `zxReport` сначала формируется целиком, затем поверх него считается контрольная сумма:
    - берётся строковое представление объекта `{ "zxReport": <zxReportJson> }`;
    - к нему применяется CRC32;
    - результат сериализуется в 8‑символьную верхнерегистровую hex-строку (`[0-9A-F]{8}`) и пишется в поле `zxReport.checksum`;
  - эта checksum используется как часть реализации требований **2.3** и **13** (контроль целостности Z‑отчётов и цепочки фискальных событий).
- **X‑отчёт (REPORT_X)**:
  - по модели использует тот же `zxReport`, что и Z‑отчёт, но без `closeShiftTime` (смена остаётся открытой);
  - благодаря фиксированным массивам и нулевым значениям X‑отчёт всегда отражает полную матрицу операций/налогов/секций за текущую смену, даже если по части измерений ещё не было движений.
